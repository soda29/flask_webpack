...
class Webpack(object):
    ...
    def javascript_tag(self, *args):
        ...
        tags = []
        for arg in args:
            assets = self.assets.get('javascript', {}).get(arg)
            if not assets:
                break
    ...
    def stylesheet_tag(self, *args):
        ...
        for arg in args:
            assets = self.assets.get('styles', {}).get(arg)
            if not assets:
                break
            if isinstance(assets, basestring):
                assets = [assets]
            ...
            for a in assets:
                tags.append(
                    '<link rel="stylesheet" href="{0}">'.format(self.asset_url_for(a)))
